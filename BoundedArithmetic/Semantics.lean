...
@[delta0_simps]
lemma realize_flip {L : Language} {M} [L.Structure M] {a b} (phi : L.Formula (a ⊕ b) ) {v : (b ⊕ a) -> M}
  : phi.flip.Realize v
    <->
...
@[delta0_simps]
lemma realize_display1 {L : Language} {M} {n1} [L.Structure M] (phi : L.Formula (Vars1 n1))  {v : ((Vars1 n1) ⊕ Empty) -> M}
...
@[delta0_simps]
lemma realize_display2 {L : Language} {M} {n1 n2} [L.Structure M] (phi : L.Formula (Vars2 n1 n2)) {v : ((Vars1 n1) ⊕ (Vars1 n2)) -> M}
...
@[delta0_simps]
lemma realize_display3 {L : Language} {M} {n1 n2 n3} [L.Structure M] (phi : L.Formula (Vars3 n1 n2 n3)) {v : ((Vars1 n1) ⊕ (Vars2 n2 n3)) -> M}
...
@[delta0_simps]
lemma realize_iAlls'.Empty {L : Language} {M} [L.Structure M] {a} (phi : L.Formula (a ⊕ Empty) ) {v : a -> M}
  : phi.iAlls'.Realize v
    <->
      phi.Realize
...
-- TODO: IT SHOULD DEFINITIONALLY?... but I couldn't prove without funext
@[delta0_simps]
lemma realize_iAlls'.Vars1 {L : Language} {M} [L.Structure M] {a} {n1} (phi : L.Formula (a ⊕ Vars1 n1) ) {v : a -> M}
  : phi.iAlls'.Realize v
    <->
      ∀ x : M, phi.Realize
...
@[delta0_simps]
lemma realize_iAlls'.Vars2 {L : Language} {M} [L.Structure M] {a} {n1 n2} (phi : L.Formula (a ⊕ Vars2 n1 n2)) {v : a -> M}
  : phi.iAlls'.Realize v
    <->
      ∀ x y : M, phi.Realize
...
@[delta0_simps]
lemma realize_iAlls'.Vars3 {L : Language} {M} [L.Structure M] {a} {n1 n2 n3} (phi : L.Formula (a ⊕ Vars3 n1 n2 n3)) {v : a -> M}
  : phi.iAlls'.Realize v
    <->
      ∀ x y z : M, phi.Realize
