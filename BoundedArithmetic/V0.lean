...
def isNum (t : num ⊕ str) [zambella.Structure (num ⊕ str)] := Structure.RelMap (L := zambella) ZambellaRel.isnum ![t]
def isStr (t : num ⊕ str) [zambella.Structure (num ⊕ str)] := Structure.RelMap (L := zambella) ZambellaRel.isstr ![t]
...
class BASIC2Model extends zambella.Structure (num ⊕ str) where
  ...
  typeStrLift :
    ∀ t : (num ⊕ str),
    isStr num str t -> t.isRight
  typeNumLift :
    ∀ t : (num ⊕ str),
    isNum num str t -> t.isLeft
  memLift (t1 : Nums num str) (t2 : Strs num str) :
    t1 ∈ t2 ↔ t1.val ∈ t2.val
...
class V0Model.{v} extends BASIC2Model num str where
  -- TODO: we can allow some more free vars here, but carefully?
  sigma0B_comp {disp : Type v} [HasDisplayed disp] {a : Type v} [IsEnum a]
...
theorem X_MIN : ∀s X > (0 : Nums num str), ∃i x <  len X, x ∈ X ∧ (∀i y < x, ¬ y ∈ X) := by
  ...
  -- (instance finding)
  let h_comp := inst.sigma0B_comp formula (disp := Vars1X) (a := Vars2yz) (by
    ...
    conv =>
      ...
      conv =>
        ...
        conv =>
          ...
          conv =>
            left
            simp only [z, instHasVar_zVars2yzX, X, instHasVar_XVars2yzX, instHasDisplayedVars1z,
          ...
          conv =>
            ...
            simp only [BoundedFormula.realize_imp]
            conv =>
              left; simp only [instHasDisplayedVars1x, z, instHasVar_zVars2yzX, Term.relabel.eq_1,
                Sum.map_inl, instHasDisplayedVars1x.eq_1, IsEnum.size.Vars1x, IsEnum.size.Vars2yz,
                instHasDisplayedVars1z, IsEnum.size.Vars1z, IsEnum.size.Vars1X, IsEnum.size.Vars1y,
                Nat.add_zero, Nat.succ_eq_add_one, Nat.reduceAdd, Fin.castAdd_zero, Fin.cast_refl,
                Function.comp_id, Equiv.coe_fn_mk, Term.realize_var, Function.comp_apply,
                Sum.swap_inl, Sum.map_inr, IsEnum.toIdx.Vars1z_z, Fin.isValue, Sum.elim_inr,
                Fin.snoc, Fin.val_eq_zero, lt_self_iff_false, ↓reduceDIte, Fin.reduceLast, cast_eq,
                Term.realize_le, Sum.elim_inl, IsEnum.toIdx.Vars1x_x, id_eq, IsEnum.toIdx.Vars2yz_z,
                Fin.coe_ofNat_eq_mod, Nat.mod_succ]
            conv =>
              right;
              unfold form1
              simp only [Formula.flip, display_x_xyzX, Term.notin, Term.in, x,
                instHasVar_xVars3X, X, instHasVar_XVars3X, instHasDisplayedVars1x,
                IsEnum.size.Vars1x, IsEnum.size.Vars2yz, instHasDisplayedVars1z, IsEnum.size.Vars1z,
                ...
                Term.realize_var, Function.comp_apply, Sum.swap_inl, Sum.map_inr,
                IsEnum.toIdx.Vars1z_z, Fin.isValue, Sum.elim_inr, Fin.snoc, Fin.val_eq_zero,
                lt_self_iff_false, ↓reduceDIte, Fin.reduceLast, cast_eq, realize_relabelEquiv,
                BoundedFormula.realize_not, BoundedFormula.realize_rel₂, Sum.elim_inl,
                IsEnum.toIdx.Vars1x_x, Sum.swap_inr, Sum.map_inl, id_eq]
      ...
      conv =>
        ...
        conv =>
          ...
          conv =>
            ...
            simp only [BoundedFormula.realize_relabelEquiv]
            simp only [Term.notin, Term.in, z, instHasVar_zVars2yzX, X, instHasVar_XVars2yzX,
      ...
      conv =>
        ...
        conv =>
          ...
          conv =>
            left
            unfold Term.in
            simp only [Language.z, instHasVar_zVars2yzX, X, instHasVar_XVars2yzX,
