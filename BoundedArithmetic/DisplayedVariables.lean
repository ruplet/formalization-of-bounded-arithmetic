...
instance (α) [HasDisplayed α] : IsEnum α where
  size := 1
  toIdx _ := 0
...
universe u
variable {L : Language} {α : Type u}

def x {α} {k} [h : HasVar_x α] : L.Term (α ⊕ Fin k) :=
  (L.var $ Sum.inl h.x)
def y {α} {k} [h : HasVar_y α] : L.Term (α ⊕ Fin k) :=
  (L.var $ Sum.inl h.y)
def z {α} {k} [h : HasVar_z α] : L.Term (α ⊕ Fin k) :=
  (L.var $ Sum.inl h.z)


attribute [simp] Function.RightInverse Function.LeftInverse

variable {disp} [HasDisplayed disp]
...
@[inline] def display_x (phi : L.Formula Vars1x) : L.Formula (Vars1x ⊕ Empty) := display phi
-- def display_x_x (phi : L.Formula Vars1x) : L.Formula (Vars1x ⊕ Empty) :=
--   phi.relabelEquiv {
--     toFun := (fun fv => match fv with
--     | .x => Sum.inl .x
--     )
...
--     right_inv := by simp only [Function.RightInverse, Function.LeftInverse, Sum.forall,
--       implies_true, reduceCtorEq, IsEmpty.forall_iff, and_self]
--   }
@[inline] def display_y (phi : L.Formula Vars1y) : L.Formula (Vars1y ⊕ Empty) := display phi
@[inline] def display_z (phi : L.Formula Vars1z) : L.Formula (Vars1z ⊕ Empty) := display phi
...
def flip {α β} (phi : L.Formula (α ⊕ β)) : L.Formula (β ⊕ α) :=
  phi.relabelEquiv {
    toFun := Sum.swap
    invFun := Sum.swap
